...
> {: lang="ja"}
>
> > [Bliss was it in that dawn to be alive,](http://www.columbia.edu/~svl2/wordsworth/ww297.html)
> >
> >（生きて迎える曙光の至福よ、）
> > {: lang="ja"}
> >
...
_that_によって導入された引用は、間接話法の中にある状態と同様にみなされ、引用符で囲まれない。
{: lang="ja"}

> Keats declares that [beauty is truth, truth beauty](http://www.columbia.edu/~svl2/keats/keats54.html).
>
...
正確なリファレンスが必要とされる学術的な著作物では、何度も現れるタイトルは省略形で示し、巻末に完全形のアルファベット順一覧を載せる。一般的な慣習として、リファレンスは括弧に入れるか脚注にするかして、本文中にそのまま書くことはしない。_act_, _scene_, _line_, _book_, _volume_, _page_（幕、場、行、冊、巻、頁）という語は、これらのうちただ1種類によって参照するとき以外は省略する。句読点は下記のように打つ。
{: lang="ja"}
